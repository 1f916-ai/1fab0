...
(L = sum sqrt(dx^2 + dy^2)), straightness, forward progress index (FPI = dx / L), and turning rate.
...
def trajectory_metrics(
    ...
    Displacement dx = x_end - x_0 and forward_progress_index (FPI = dx / L) are signed
    ...
    dict
        ...
        - 'forward_progress_index': float (displacement / path_length, bounded in [-1.0, 1.0])
    ...
    if path_length > 1e-9:
        forward_progress_index = float(displacement / path_length)
    ...
    metrics = {
        ...
        'forward_progress_index': forward_progress_index,
        'fpi': forward_progress_index,
        'chemotaxis_index': forward_progress_index,
