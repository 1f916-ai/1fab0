...
ids = None; n = 0; sign = None; A = None; ann = None; typ = None; side = None; rec = None

def load_substrate():
    global ids, n, sign, A, ann, typ, side, rec
    if ids is not None: return
    if sp is None or f is None:
        raise RuntimeError("scipy, torch, and pyarrow are required to load G_traced substrate.")
...
def item_plan(it):
    ...
    return stim, ro
def main():
    ap = argparse.ArgumentParser(); ap.add_argument('--items', default='1,2,3,4,5,6'); ap.add_argument('--trials', type=int, default=T['paired_trials']); ap.add_argument('--conditions', default='real,shuffled,random'); ap.add_argument('--smoke', action='store_true'); ap.add_argument('--simulator', choices=['spiking', 'arena'], default='spiking', help='Simulation backend: spiking LIF (Shiu et al. 2024) or arena continuous rate ODE'); ap.add_argument('--rate-sweep', default=None, help='v3: comma-separated multipliers of the reference probe rate; the random twin is calibrated to each within 25%% and every step is scored'); ap.add_argument('--activity-match', action='store_true', help='v2: calibrate the random twin so its probe population rate is within [0.5,2]x the reference model'); ap.add_argument('--seed-material', default=None, help='sealhash:checkpointroot — derive trial seeds as sha256(seal || root || i) (vish, c60643)'); ap.add_argument('--out', default=None, help='Output runs JSONL path (default: results/runs-arena.jsonl if --simulator arena, else results/runs.jsonl)')
    a = ap.parse_args()
    if a.out is None:
        a.out = 'results/runs-arena.jsonl' if a.simulator == 'arena' else 'results/runs.jsonl'
    load_substrate()
    def trial_seed(i):
    ...
    W_real = build_weights(A) if a.simulator == 'spiking' else None; shuffles = {}
    ref_rate = None
    if a.simulator == 'spiking' and (a.activity_match or a.rate_sweep):
        ...
        for it in items:
            ...
            for cond in conds:
                for tr in range(ntr):
                    sd = trial_seed(tr)
                    arena_sim = None
                    if a.simulator == 'arena':
                        arena_sim = ConnectomeArenaSimulation(
                            weights=A,
                            signs=sign,
                            num_neurons=n,
                            neuron_types=typ,
                            ...
                            condition=cond,
                            seed=sd,
                            tau_ms=R.get('T_mbr_ms', 20.0),
                            r_max=150.0,
                            activation='sigmoid'
                        )
                        P0 = dict(simulator='arena', condition=cond, seed=sd, tau=float(R.get('T_mbr_ms', 20.0)), r_max=150.0)
                    else:
                    ...
                    for step in (steps if (cond == 'random' and a.simulator == 'spiking') else [None]):
                      if step is not None:
                      ...
                      elif a.simulator == 'spiking': P0 = P
                      for sname, inputs in stim.items():
                          t1 = time.time()
                          is_walking = it['id'] in (1, 2, 3)
                          if a.simulator == 'arena':
                              res, traj_metrics = arena_sim.simulate(
                                  inputs=inputs,
                                  readouts=ro,
                                  warmup_ms=T['warmup_ms'],
                                  baseline_ms=T['baseline_ms'],
                                  stimulus_ms=T['stimulus_ms'],
                                  dt_ms=R.get('dt_ms', 1.0),
                                  track_trajectory=is_walking
                              )
                          else:
                              res, traj_metrics = simulate(W, P0, jit, inputs, ro, seed=sd, track_trajectory=is_walking)
